...
async function getStockDescription(symbol) {
  ...
  const url = `https://www.alphavantage.co/query?function=OVERVIEW&symbol=${symbol}&apikey=${ALPHAVANTAGE_API_KEY}`; // call api

  let res = await fetch(url);
  let data = await res.json();
  let sector =
    data.Sector.substring(0, 1) + data.Sector.substring(1).toLowerCase(); // format string (initially, text is all caps)
  let industry =
    data.Industry.substring(0, 1) + data.Industry.substring(1).toLowerCase(); // format string (initially, text is all caps)
