  ...
  const handleLogin = () => {
    ...
    if (email !== "" && password !== "" && !emailError && !passwordError) {
      ...
      if (email === "" || password === "" || emailError || passwordError) {
        return Alert.alert(
          "Error",
          "Your email and/or password do not follow the appropriate formats. Please fix and try again."
        );
      }
    }
  };
  ...
  return (
    <TouchableOpacity>
      ...
      <HelperText type="error" visible={passwordError} style={styles.helper}>
        Passwords should be at least 6 characters in length and include at least
        one number
      </HelperText>
      <View style={styles.break}></View>
      <Button
